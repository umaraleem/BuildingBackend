...
console.log(req.query.AddressId === 'null');
    if (req.query.AddressId === 'null')
    {
        ...
        connection.query('INSERT INTO Address SET ?', userdata, (err, result) => {
            ...
            // res.redirect("https://buildingsrecordsystem.netlify.app/?register=true");
          });
          ...
          connection.query(query, (error, results, fields) => {
            if (error) {
              ...
              return;
                ...
                connection.query(`Update Dealer SET AddressId = ${AddressId}`,(err, result) => {
                    if (err) throw err;
                    console.log('Data inserted successfully!');
                    console.log(result);
                    // send a response to the client
                    console.log("inserted");
                    res.redirect(`https://buildingsrecordsystem.netlify.app/DealerSpecificProduct?ProductId=${req.query.ProductId}&show=1`);
                  });
